...
function jornadasView(){if(!currentPool)return poolsView();const rounds=jornadaRows();return `<div class="card classification-card"><h2>Jornadas</h2><p class="muted">Asi quedo la clasificacion al cerrar cada jornada. Sirve para premios y sanciones jornada a jornada.</p>${rounds.length?rounds.slice().reverse().map(item=>{const best=[...item.rows].sort((a,b)=>b.roundPts-a.roundPts||b.total-a.total)[0];const worst=[...item.rows].sort((a,b)=>a.roundPts-b.roundPts||a.total-b.total)[0];return `<div class="round-standing"><h3>${safe(item.round.key)}</h3><div class="round-awards"><span>Premio: <b>${safe(best?.u?.nick||'-')} +${best?.roundPts??0}</b></span><span>Sancion: <b>${safe(worst?.u?.nick||'-')} +${worst?.roundPts??0}</b></span></div>${item.rows.map((r,i)=>`<div class="round-row"><div><b>${i+1}</b></div><div>${avatar(r.u)}<b>${safe(r.u.nick)}</b></div><div><span>+${r.roundPts}</span><small>jornada</small></div><div><span>${r.total}</span><small>total</small></div></div>`).join('')}</div>`}).join(''):'<p class="muted">Todavia no hay ninguna jornada cerrada con todos sus resultados.</p>'}${classificationTabs()}</div>`}
