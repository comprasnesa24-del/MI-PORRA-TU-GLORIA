...
function advanceSelectHtml(m){if(!KNOCKOUT_LINKS[String(m.id)])return'';return `<label>Pasa eliminatoria</label><select id="adv_${m.id}" class="score-select"><option value="">Solo si hay empate</option><option value="${safe(m.home_team)}" ${normTeam(m.advance_team)===normTeam(m.home_team)?'selected':''}>${teamName(m.home_team)}</option><option value="${safe(m.away_team)}" ${normTeam(m.advance_team)===normTeam(m.away_team)?'selected':''}>${teamName(m.away_team)}</option></select>`}async function propagateKnockoutNames(startId,realData){
...
Object.assign(window,{login,register,logout,setTab,changePool,selectPool,createPool,joinPool,leavePool,deletePool,savePrediction,saveGlobalReal,resetGlobalReal,updatePoolSettings,saveProfile,changePassword,sendMessage,refreshChat,savePodiumPrediction,saveRealPodium,showPodiumNow})
