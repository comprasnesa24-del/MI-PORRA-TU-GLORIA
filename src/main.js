...
function podiumPredictionHtml(){const pm=myMembership();if(!currentPool||!pm||currentUser?.role==='admin')return'';const closed=podiumClosed();return `<div class="card podium-card"><h2>Podio del Mundial</h2><p class="muted">Campeon +20 · Segundo +15 · Tercero +10<br>Hay que guardarlo antes del ${podiumDeadlineText()}.</p>${closed?'<div class="notice"><b>Plazo cerrado.</b> Ya no se puede modificar el podio.</div>':''}<div class="podium-grid"><label>Campeon${teamPickSelectHtml('pick_champion',pm.champion_pick,closed)}</label><label>Segundo${teamPickSelectHtml('pick_second',pm.second_pick,closed)}</label><label>Tercero${teamPickSelectHtml('pick_third',pm.third_pick,closed)}</label></div>${closed?'':`<button class="yellow" onclick="window.savePodiumPrediction()">Guardar podio</button>`}</div>`}
...
function tabs(){
  ...
  const userCard=`<div class="card profile-card"><h2>${avatar(currentUser)} Hola, ${safe(currentUser.nick)}</h2><p>${currentUser?.role==='admin'?'Modo administrador':'Pepe es tu lider'}</p><button class="small yellow" onclick="window.saveProfile()">Editar avatar</button></div>`;
...
Object.assign(window,{login,register,logout,setTab,changePool,selectPool,createPool,joinPool,leavePool,deletePool,savePrediction,saveGlobalReal,resetGlobalReal,updatePoolSettings,saveProfile,sendMessage,refreshChat,savePodiumPrediction,saveRealPodium,showPodiumNow})
