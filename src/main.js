...
function pred(mid) {
  return predictions.find(p => p.user_id === currentUser?.id && p.match_id === mid)
}
...
async function recoverPassword() {
  ...
  alert(`Contraseña de ${res.data.nick}: ${res.data.password}\n\nNota: esto es solo para la versión sencilla. Más adelante se puede hacer recuperación segura por email.`)
}
...
async function savePrediction(mid) {
  ...
  if (existing) {
    ...
    await supabase
      .from('predictions')
      .insert({ user_id: currentUser.id, match_id: mid, pred_home: ph, pred_away: pa })
  }
...
async function saveWinnerHistory() {
  ...
  const ranked = users.map(u => ({ ...u, ...userStats(u.id) }))
  ...
  app.innerHTML = `
    <div class="app">
      ...
      <div class="card">
        ...
        <p>Configura estas variables en Vercel:</p>
        <div class="notice">
...
function loginView() {
  return `
    <div class="card">
      <h2>Entrar</h2>
      <p class="muted">Primero registra un usuario. Para ser administrador usa el nick <b>admin</b>.</p>
...
function tabs() {
  return `
    <div class="tabs">
      ...
      ${currentUser?.role === 'admin' ? `<button class="yellow" onclick="window.setTab('admin')">Admin</button>` : ''}
  ...
  return `
    <div class="card">
      ...
      ${matches.map(m => {
        ...
        return `
          <div class="match">
            <span class="group">Grupo ${safe(m.group_name)}</span>
            <div class="teams">${safe(m.home_team)} vs ${safe(m.away_team)}</div>
            ...
            <div class="score">
              <div><label>${safe(m.home_team)}</label><input ${locked && currentUser.role !== 'admin' ? 'disabled' : ''} type="number" min="0" id="ph_${m.id}" value="${p?.pred_home ?? ''}"></div>
              <div style="font-weight:900;padding-bottom:17px">-</div>
              <div><label>${safe(m.away_team)}</label><input ${locked && currentUser.role !== 'admin' ? 'disabled' : ''} type="number" min="0" id="pa_${m.id}" value="${p?.pred_away ?? ''}"></div>
            </div>
...
function rulesView() {
  return `
    <div class="card">
      ...
      <div class="rule"><h3>⚽ Diferencia correcta: 3 puntos</h3><p>No es exacto, pero aciertas la diferencia de goles. Ejemplo: queda 1-0 y pusiste 2-1.</p></div>
      <div class="rule"><h3>✅ Signo correcto 1 / X / 2: 2 puntos</h3><p>Acertar ganador o empate, aunque no sea exacto ni diferencia.</p></div>
...
function rankingView() {
  const rows = users.map(u => ({ ...u, ...userStats(u.id) }))
    .sort((a, b) => b.total - a.total || b.exact - a.exact || b.diff - a.diff || b.sg - a.sg)

  return `
    <div class="card">
      <h2>Clasificación privada</h2>
      ${rows.map((r, i) => {
  ...
  return `
    <div class="card">
      ...
      <div style="margin-bottom:20px">
        <button class="small blue" onclick="window.copyInviteLink()">📲 Copiar enlace de invitación</button>
        ...
        <button class="small red" onclick="window.deleteAllPredictions()">🗑️ Borrar TODOS los pronósticos</button>
      </div>
      ...
      ${users.map(u => {
        ...
        return `
          <div class="ranking">
            <div>${u.role === 'admin' ? '👑' : '👤'}</div>
            <div>
              ${safe(u.nick)}<br>
              <span class="muted">${safe(u.email || 'Sin email')} · ${u.role} · ${s.total} pts</span>
            </div>
            <div>
              <button class="small yellow" onclick="window.toggleAdmin('${u.id}', '${u.role}', '${safe(u.nick)}')">${u.role === 'admin' ? 'Quitar admin' : 'Hacer admin'}</button>
              <button class="small red" onclick="window.resetUserPredictions('${u.id}', '${safe(u.nick)}')">Reset puntos</button>
              ${u.nick.toLowerCase() !== 'admin' ? `<button class="small red" onclick="window.deleteUser('${u.id}', '${safe(u.nick)}')">Eliminar</button>` : ''}
            </div>
          </div>
        `
      }).join('')}
      ...
      <h3>⚽ Resultados</h3>
      <p class="muted">Introduce resultados reales. También puedes reiniciar resultados o borrar pronósticos de prueba.</p>
      ...
      ${matches.map(m => `
        <div class="adminrow">
          ...
          <div class="muted">${new Date(m.match_date).toLocaleString('es-ES')}</div>
          <div class="score">
            <div><label>${safe(m.home_team)}</label><input type="number" min="0" id="rh_${m.id}" value="${m.real_home ?? ''}"></div>
            <div style="font-weight:900;padding-bottom:17px">-</div>
            <div><label>${safe(m.away_team)}</label><input type="number" min="0" id="ra_${m.id}" value="${m.real_away ?? ''}"></div>
          </div>
