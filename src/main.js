...
function jornadaRows(){const usersInPool=poolUsers().filter(u=>u.role!=='admin');const totals=new Map(usersInPool.map(u=>[u.id,0]));return competitionRounds().filter(r=>r.finished).map(round=>{const rows=usersInPool.map(u=>{const roundPts=roundPredictionPointsForUser(u,round)+roundPerfectBonusForUser(u,round);const total=(totals.get(u.id)||0)+roundPts;totals.set(u.id,total);return{u,roundPts,total}}).sort((a,b)=>b.roundPts-a.roundPts||a.total-b.total||String(a.u.nick).localeCompare(String(b.u.nick)));return{round,rows}})}
...
function myLeaguePredictionView(){if(!currentPool)return poolsView();if(currentCompetition!=='liga')return rankingView();const pm=myMembership();const pick=parseLeaguePick(pm);return `<div class="card classification-card"><h2>Mi pronostico</h2>${leagueStarted()?'<div class="notice"><b>Clasificacion guardada.</b> Ya no se puede modificar.</div>':'<div class="notice"><b>Todavia puedes modificarla en Pendientes.</b></div>'}${pick.length?`<div class="league-pick-list">${pick.map((t,i)=>`<div><b>${i+1}</b><span>${safe(t)}</span></div>`).join('')}</div>`:'<p class="muted">Aun no has guardado tu clasificacion.</p>'}${classificationTabs()}</div>`}
function jornadasView(){if(!currentPool)return poolsView();const rounds=jornadaRows();const ledger=moneyLedger();return `<div class="card classification-card"><h2>Jornadas</h2><p class="muted">Asi quedo la clasificacion al cerrar cada jornada. Sirve para premios y sanciones jornada a jornada.</p>${roundMoneySettingsHtml()}${moneyLedgerHtml(ledger)}${rounds.length?rounds.slice().reverse().map(item=>{const rows=ledger.byRound.get(item.round.key)||item.rows;const best=[...rows].sort((a,b)=>b.roundPts-a.roundPts||a.total-b.total)[0];const worst=[...rows].sort((a,b)=>a.roundPts-b.roundPts||b.total-a.total)[0];return `<div class="round-standing"><h3>${safe(item.round.key)}</h3><div class="round-awards"><span>Premio: <b>${safe(best?.u?.nick||'-')} +${best?.roundPts??0}</b></span><span>Sancion: <b>${safe(worst?.u?.nick||'-')} +${worst?.roundPts??0}</b></span></div>${rows.map((r,i)=>`<div class="round-row money-round-row"><div><b>${i+1}</b></div><div>${avatar(r.u)}<b>${safe(r.u.nick)}</b></div><div><span>+${r.roundPts}</span><small>jornada</small></div><div><span>${r.total}</span><small>total</small></div><div><span class="${(r.balance||0)>=0?'money-good':'money-bad'}">${moneyFmt(r.balance||0)}</span><small>dinero</small></div></div>`).join('')}</div>`}).join(''):'<p class="muted">Todavia no hay ninguna jornada cerrada con todos sus resultados.</p>'}${classificationTabs()}</div>`}
