...
function leagueTablePointsForMember(pm){if(!pm||!leagueFinished())return 0;const pick=parseLeaguePick(pm);const real=actualLeagueTable();let pts=0;real.forEach((row,i)=>{if(normTeam(pick[i])===normTeam(row.team))pts+=i===0?20:i===1?15:i===2?10:3});return pts}
function leagueTableSelectHtml(pos,value,disabled){return `<select id="league_pos_${pos}" class="league-team-select" ${disabled?'disabled':''}><option value="">${pos}</option>${leagueTeams().map(t=>`<option value="${safe(t)}" ${normTeam(value)===normTeam(t)?'selected':''}>${pos}. ${safe(t)}</option>`).join('')}</select>`}
function leagueTablePredictionHtml(){if(!currentPool||currentCompetition!=='liga'||currentUser?.role==='admin')return'';const pm=myMembership();if(!pm)return'';const teams=leagueTeams();if(!teams.length)return'';const closed=leagueStarted();const pick=parseLeaguePick(pm);return `<div class="card league-table-card"><h2>Clasificación hipotética de Liga</h2><p class="muted">Ordena la Liga antes del primer partido. Puntos al final: campeón +20, segundo +15, tercero +10 y cada puesto exacto del 4 al 20 suma +3.</p>${closed?'<div class="notice"><b>Plazo cerrado.</b> Ya no se puede modificar.</div>':''}<div class="league-table-grid">${teams.map((_,i)=>leagueTableSelectHtml(i+1,pick[i],closed)).join('')}</div>${closed?'':`<button class="yellow" onclick="window.saveLeagueTablePrediction()">Guardar clasificación</button>`}${leagueTableSummaryHtml()}</div>`}
function leagueTableSummaryHtml(){if(!currentPool||currentCompetition!=='liga')return'';const rows=poolUsers().filter(u=>u.role!=='admin').map(u=>({u,pm:poolMembers.find(pm=>pm.user_id===u.id&&pm.pool_id===currentPool.id)}));if(!leagueStarted()){const done=rows.filter(r=>leaguePickComplete(r.pm)).length;return `<div class="notice compact-card"><b>${done}/${rows.length}</b> usuarios ya guardaron su clasificación.</div>`}return `<div class="notice compact-card"><h3>Clasificaciones guardadas</h3>${rows.map(r=>`<p><b>${safe(r.u.nick)}:</b> ${leaguePickComplete(r.pm)?'Guardada':'Sin guardar'} ${leagueFinished()?`(${leagueTablePointsForMember(r.pm)} pts)`:''}</p>`).join('')}</div>`}
...
function tabs(){
  ...
  const userCard=`<div class="card profile-card"><h2>${avatar(currentUser)} Hola, ${safe(currentUser.nick)}</h2><p>${currentUser?.role==='admin'?'Modo administrador':'Pepe es tu lider'}</p><button class="small yellow" onclick="window.saveProfile()">Editar avatar</button><button class="small blue" onclick="window.changePassword()">Cambiar contraseña</button></div>`;
...
function rulesView(){if(!currentPool)return poolsView();return `<div class="card classification-card"><h2>Normas</h2><div class="rule"><h3>Exacto: 5 puntos</h3></div><div class="rule"><h3>Diferencia correcta: 3 puntos</h3></div><div class="rule"><h3>Signo: 2 puntos</h3></div><div class="rule"><h3>Bonus Anti-Copia</h3><p>Si aciertas exacto y pocos pusieron lo mismo: único +3, dos iguales +2, tres iguales +1, cuatro o más +0.</p></div><div class="rule"><h3>Pleno de jornada</h3><p>Si aciertas el signo de todos los partidos de una jornada de Liga, +20 puntos.</p></div>${currentCompetition==='liga'?'<div class="rule"><h3>Clasificación hipotética de Liga</h3><p>Se guarda antes del primer partido. Al final: campeón +20, segundo +15, tercero +10 y cada puesto exacto del 4 al 20 suma +3.</p></div>':''}<div class="rule"><h3>Joker</h3><p>${currentPool.enable_joker?'5 Jokers por usuario. Puntuan doble.':'Desactivado.'}</p></div><div class="rule"><h3>Goleador</h3><p>${currentPool.enable_scorer?'Si aciertas un goleador, +2 puntos. Dejarlo vacio da 0 puntos, salvo 0-0 con sin goleador.':'Desactivado.'}</p></div><div class="rule"><h3>MVP</h3><p>${currentPool.enable_mvp?'Si aciertas el MVP del partido, +2 puntos.':'Desactivado.'}</p></div><div class="rule"><h3>Posible expulsion</h3><p>${currentPool.enable_sent_off?'Si eliges y aciertas el jugador expulsado, +2 puntos. No apostar da 0 puntos.':'Desactivado.'}</p></div>${classificationTabs()}</div>`}
...
Object.assign(window,{login,register,logout,setTab,changePool,setCompetition,selectPool,createPool,joinPool,leavePool,deletePool,savePrediction,saveGlobalReal,resetGlobalReal,updatePoolSettings,saveProfile,changePassword,sendMessage,refreshChat,savePodiumPrediction,saveRealPodium,saveLeagueTablePrediction,showPodiumNow})
