...
function jornadasView(){if(!currentPool)return poolsView();const finishedRounds=jornadaRows();const ledger=moneyLedger();const realRounds=competitionRounds();const allRounds=Array.from({length:currentCompetition==='liga'?38:Math.max(realRounds.length,1)},(_,i)=>realRounds.find(r=>roundOrderKey(r.key)===i+1)||{key:'Jornada '+(i+1),matches:[],finished:false,firstDate:0,pending:true});const selected=selectedRoundKey?allRounds.find(r=>r.key===selectedRoundKey):activeRoundFrom(allRounds);if(selected)selectedRoundKey=selected.key;const ledgerRows=selected?ledger.byRound.get(selected.key):null;const rows=ledgerRows||roundRowsForDisplay(selected);const best=[...rows].sort((a,b)=>b.roundPts-a.roundPts||a.total-b.total)[0];const worst=[...rows].sort((a,b)=>a.roundPts-b.roundPts||b.total-a.total)[0];const selectedHasResult=roundHasResult(selected);return `<div class="card classification-card"><h2>Clasificacion de jornada</h2><p class="muted">Por defecto ves la jornada activa. La clasificacion completa esta en la pestana General.</p>${classificationTabs()}<div class="round-tab-grid">${allRounds.map(item=>`<button class="${item.key===selectedRoundKey?'active':''} ${item.matches?.length?'':'pending'}" onclick="window.setRoundTab('${safe(item.key)}')">${safe(roundShortLabel(item.key))}</button>`).join('')}</div>${selected?`<div class="round-standing"><h3>${safe(selected.key)}${selected.pending?' · pendiente':''}</h3>${selected.matches?.length?`<div class="round-awards"><span>Mejor: <b>${safe(best?.u?.nick||'-')} +${best?.roundPts??0}</b></span><span>Peor: <b>${safe(worst?.u?.nick||'-')} +${worst?.roundPts??0}</b></span></div>${rows.map((r,i)=>`<div class="round-row money-round-row"><div><b>${i+1}</b></div><div>${avatar(r.u)}<b>${safe(r.u.nick)}</b></div><div><span>+${r.roundPts}</span><small>jornada</small></div><div><span>${r.total}</span><small>general</small></div>${ledgerRows?`<div><span class="${(r.balance||0)>=0?'money-good':'money-bad'}">${moneyFmt(r.balance||0)}</span><small>dinero</small></div>`:''}</div>`).join('')}${!selected.finished&&!selectedHasResult?'<p class="muted">Aun no hay resultados en esta jornada.</p>':''}`:'<p class="muted">Esta jornada aun no tiene partidos cargados.</p>'}</div>`:''}${roundMoneySettingsHtml()}${moneyLedgerHtml(ledger)}</div>`}
