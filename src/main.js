...
const CHAT_NOTIFY_KEY='mi_porra_chat_notify_v1'
const COMPETITIONS={mundial:{id:'mundial',name:'Mundial 2026',short:'Mundial'},liga:{id:'liga',name:'Liga BBVA 2026/27',short:'Liga BBVA'},primera_rfef_g1:{id:'primera_rfef_g1',name:'Primera Federacion Grupo 1 2026/27',short:'1ª RFEF G1'},primera_rfef_g2:{id:'primera_rfef_g2',name:'Primera Federacion Grupo 2 2026/27',short:'1ª RFEF G2'},mix:{id:'mix',name:'MIX multideporte',short:'MIX'}}
const LEAGUE_COMPETITION_IDS=['liga','primera_rfef_g1','primera_rfef_g2']

let currentUser=null,currentPool=null,currentCompetition='mundial',tab='porras',selectedRoundKey='',selectedHistoryUserId=''
...
let chatChannel=null,chatChannelKey=''
...
function isLeagueCompetition(id=currentCompetition){return LEAGUE_COMPETITION_IDS.includes(id)}
...
function jokerLimit(){return isLeagueCompetition()?20:5}
function jokerEnabled(){return isLeagueCompetition()||!!currentPool?.enable_joker}
...
function chatNotificationsSupported(){return typeof Notification!=='undefined'}
function chatNotificationsActive(){return chatNotificationsSupported()&&Notification.permission==='granted'&&localStorage.getItem(CHAT_NOTIFY_KEY)==='1'}
function chatNotificationControlsHtml(){
  if(!chatNotificationsSupported())return `<div class="notice compact-card"><b>Notificaciones:</b> este movil/navegador no las permite aqui.</div>`;
  const active=chatNotificationsActive();
  const blocked=Notification.permission==='denied';
  return `<div class="notice compact-card chat-notice"><b>Notificaciones de chat:</b> ${active?'Activadas':blocked?'Bloqueadas':'Desactivadas'}<br><span class="muted">Te avisara cuando hablen en el chat de esta porra.</span><br>${active?`<button class="small red" onclick="window.disableChatNotifications()">Desactivar</button>`:`<button class="small yellow" onclick="window.enableChatNotifications()" ${blocked?'disabled':''}>Activar notificaciones</button>`}${blocked?'<p class="muted">Estan bloqueadas en el navegador. Hay que permitirlas desde ajustes del sitio.</p>':''}</div>`;
}
async function enableChatNotifications(){
  if(!chatNotificationsSupported())return alert('Este movil o navegador no permite notificaciones aqui.');
  const permission=Notification.permission==='granted'?'granted':await Notification.requestPermission();
  if(permission!=='granted')return alert('No se han permitido las notificaciones.');
  localStorage.setItem(CHAT_NOTIFY_KEY,'1');
  setupChatRealtime();
  render();
  alert('Notificaciones de chat activadas.');
}
function disableChatNotifications(){localStorage.setItem(CHAT_NOTIFY_KEY,'0');render()}
function notifyChatMessage(m){
  if(!chatNotificationsActive()||!m)return;
  if(tab==='chat'&&document.visibilityState==='visible')return;
  const u=users.find(x=>String(x.id)===String(m.user_id));
  const body=(safe(u?.nick||'Usuario')+': '+safe(messageText(m))).replace(/<[^>]*>/g,'').slice(0,140);
  try{new Notification('Nuevo mensaje en '+(currentPool?.name||'la porra'),{body,tag:'chat_'+currentPool?.id,icon:'/assets/icon-192.png',badge:'/assets/icon-192.png'})}catch(e){}
}
function removeChatRealtime(){
  if(chatChannel&&supabase){try{supabase.removeChannel(chatChannel).catch(()=>{})}catch(e){}}
  chatChannel=null;chatChannelKey='';
}
function setupChatRealtime(){
  if(!supabaseReady||!supabase||!currentUser||!currentPool){removeChatRealtime();return}
  const key=currentPool.id+'_'+currentUser.id;
  if(chatChannel&&chatChannelKey===key)return;
  removeChatRealtime();
  chatChannelKey=key;
  chatChannel=supabase.channel('chat_messages_'+currentPool.id+'_'+currentUser.id)
    .on('postgres_changes',{event:'INSERT',schema:'public',table:'messages',filter:'pool_id=eq.'+currentPool.id},payload=>{
      const m=payload.new||{};
...
function leagueMatches(){return isLeagueCompetition()?matches.filter(m=>competitionOf(m)===currentCompetition):[]}
...
function leagueTableDeadlineText(pm){const deadline=leagueTableDeadlineForMember(pm);return deadline?new Date(deadline).toLocaleString('es-ES'):'el inicio de '+competitionName()}
...
function loginView(){return `<div class="app login-screen"><div class="hero"><div><div class="kicker">MUNDIAL · APP PRIVADA</div><h1>Mi Porra<br><span>Tu Gloria</span></h1><p>Acierta el marcador y presume.</p></div><div class="ball">⚽</div></div><div class="card login-card"><h2>Entrar</h2><label>Nick</label><input id="nick" autocomplete="username" autocapitalize="none" spellcheck="false"><label>Email opcional</label><input id="email" type="email" autocomplete="email" autocapitalize="none" spellcheck="false"><label>Contraseña</label><input id="pass" type="password" autocomplete="current-password" autocapitalize="none" spellcheck="false"><div class="login-actions"><button onclick="window.login()">Entrar</button><button class="blue" onclick="window.register()">Registrarme</button></div><p class="muted login-help">Nick sin importar mayusculas ni acentos.</p><p class="muted login-help"><a href="/privacy.html" target="_blank">Politica de privacidad</a></p></div></div>`}
...
function logout(){removeChatRealtime();currentUser=null;currentPool=null;tab='porras';selectedHistoryUserId='';clearSession();render()}
function setTab(t){tab=t;if(t==='jornadas')selectedRoundKey='';if(t==='chat')markChatRead();saveSession();render()}
function changePool(){removeChatRealtime();currentPool=null;tab='porras';selectedHistoryUserId='';saveSession();render()}
function setCompetition(id){if(!COMPETITIONS[id])return;removeChatRealtime();currentCompetition=id;currentPool=null;tab='porras';selectedHistoryUserId='';saveSession();render()}
...
function tabs(){
  ...
  return currentUser?.role==='admin'?`${topbar}${adminTabs}${userCard}${poolLabel}`:`${topbar}${poolLabel}${userBottom}`;
}
...
function autoCopyNoticeHtml(){if(!currentPool||currentUser?.role==='admin')return'';return `<div class="notice compact-card autocopy-inline"><div><b>Autocopia:</b> ${autoCopyEnabled()?'Activada':'Desactivada'}<br><span class="muted">Copia este marcador a tus otras porras de la misma competicion.</span></div><button class="small yellow" onclick="window.toggleAutoCopyPredictions()">${autoCopyEnabled()?'Desactivar':'Activar'}</button></div>`}
...
function poolsView(){const myIds=currentUser?.role==='admin'?pools.map(p=>p.id):poolMembers.filter(pm=>pm.user_id===currentUser.id).map(pm=>pm.pool_id);const list=pools.filter(p=>myIds.includes(p.id)&&competitionOf(p)===currentCompetition);return `<div class="card pool-home compact-pool-home"><div class="pool-home-title"><h2>${currentUser?.role==='admin'?'Todas las porras':'Elige porra'}</h2>${currentUser?.role!=='admin'?`<button class="small blue" onclick="window.changePassword()">Contrasena</button>`:''}</div>${competitionSwitcherHtml()}${currentUser?.role!=='admin'?`<div class="pool-actions compact-pool-actions"><button onclick="window.createPool()">Crear porra</button><button class="yellow" onclick="window.joinPool()">Unirme</button></div>`:''}${currentCompetition==='mix'?'<div class="notice compact-card"><b>MIX:</b> campeones de varias competiciones.</div>':''}${list.map(p=>`<div class="match pool-choice compact-pool-choice"><div class="pool-choice-main"><div><h3>${safe(p.name)}</h3><p><b>${safe(competitionName(competitionOf(p)))}</b> · Codigo <b>${safe(p.code)}</b> · ${poolMembers.filter(pm=>pm.pool_id===p.id).length} usuarios</p></div><button onclick="window.selectPool('${p.id}')">Entrar</button></div>${p.prizes?`<p class="pool-prize"><b>Premios:</b> ${safe(p.prizes).replace(/\n/g,' · ')}</p>`:''}${poolPrizeButton(p)}${currentUser?.role==='admin'?`<button class="red" onclick="window.deletePool('${p.id}')">Eliminar</button>`:''}${currentUser?.role!=='admin'?`<details class="leave-pool-box"><summary>Opciones de salida</summary><button class="small red" onclick="window.leavePool('${p.id}')">Salir de esta porra</button></details>`:''}</div>`).join('')||'<p class="muted">No tienes porras todavia en esta competicion.</p>'}</div>`}
function selectPool(id){currentPool=pools.find(p=>String(p.id)===String(id))||null;if(currentPool)currentCompetition=competitionOf(currentPool);tab=currentUser?.role==='admin'?'admin':'partidos';selectedHistoryUserId='';setupChatRealtime();saveSession();render()}
async function createPool(){if(currentUser?.role==='admin')return alert('El admin no juega');const name=prompt('Nombre de la porra privada:');if(!name)return;let code=prompt('Codigo invitacion. Vacio para automatico:');if(!code)code=generateCode();code=code.trim().toUpperCase();const ex=await supabase.from('pools').select('*').eq('code',code).maybeSingle();if(ex.data)return alert('Ese codigo ya existe');const isLiga=isLeagueCompetition(),isMix=currentCompetition==='mix';const enable_joker=isLiga?true:(isMix?false:confirm('Activar Joker? Cada usuario tendra 5 Jokers.'));const enable_scorer=isLiga||isMix?false:confirm('Activar goleador? Si acierta, +2 puntos.');const enable_mvp=isLiga||isMix?false:confirm('Activar MVP del partido? Si acierta, +2 puntos.');const enable_sent_off=isLiga||isMix?false:confirm('Activar posible expulsion? Si acierta, +2 puntos.');const prizes=prompt('Premios de esta porra:','')||'';const poolData={name,code,created_by:currentUser.id,enable_joker,enable_scorer,enable_mvp,enable_sent_off,prizes,competition:currentCompetition};let ins=await supabase.from('pools').insert(poolData).select().single();if(ins.error&&/competition|schema cache|column/i.test(ins.error.message)){delete poolData.competition;ins=await supabase.from('pools').insert(poolData).select().single()}if(ins.error)return alert(ins.error.message);await supabase.from('pool_members').insert({pool_id:ins.data.id,user_id:currentUser.id,role:'admin'});currentPool=ins.data;currentCompetition=competitionOf(currentPool);tab='partidos';saveSession();await loadData()}
...
function roundPerfectBonusForUser(u,round){if(!isLeagueCompetition()||!round.finished)return 0;const ok=round.matches.every(m=>{const p=poolPredictions().find(pr=>pr.user_id===u.id&&String(pr.match_id)===String(m.id));return p&&sign(p.pred_home,p.pred_away)===sign(m.real_home,m.real_away)});return ok?20:0}
...
function roundHasResult(round){return !!round?.matches?.some(m=>m.real_home!==null&&m.real_away!==null)}
function activeRoundFrom(allRounds){const realRounds=allRounds.filter(r=>r.matches?.length);const now=Date.now();return realRounds.find(r=>!r.finished&&r.matches.some(m=>roundHasResult(r)||new Date(m.match_date).getTime()<=now))||realRounds.find(r=>!r.finished&&r.firstDate>=now)||[...realRounds].reverse().find(r=>r.finished)||allRounds[0]}
function roundRowsForDisplay(round){const usersInPool=poolUsers().filter(u=>u.role!=='admin');return usersInPool.map(u=>{const roundPts=round?.matches?.length?roundPredictionPointsForUser(u,round)+roundPerfectBonusForUser(u,round):0;return{u,roundPts,total:userStats(u).total}}).sort((a,b)=>b.roundPts-a.roundPts||a.total-b.total||String(a.u.nick).localeCompare(String(b.u.nick)))}
...
function roundMoneyLedgerHtml(rows){if(!rows?.length||!roundMoneyRules().length)return'';const lines=rows.map((r,i)=>({...r,roundMoney:roundMoneyForIndex(i,rows.length)}));const bank=-(lines.reduce((sum,r)=>sum+r.roundMoney,0));const bankLine=Math.abs(bank)>0.004?`<div class="bank-row"><span><b>BANCO</b></span><b class="${bank>=0?'money-good':'money-bad'}">${moneyFmt(bank)}</b></div>`:'';return `<div class="notice compact-card money-ledger"><h3>Cuenta de esta jornada</h3>${lines.map(r=>`<div><span>${avatar(r.u)}<b>${safe(r.u.nick)}</b></span><b class="${r.roundMoney>=0?'money-good':'money-bad'}">${moneyFmt(r.roundMoney)}</b></div>`).join('')}${bankLine}</div>`}
...
function classificationTabs(){const mine=isLeagueCompetition()&&leagueStarted()?`<button class="${tab==='mi_pronostico'?'active':''}" onclick="window.setTab('mi_pronostico')">Mi pronostico</button>`:'';return `<div class="section-tabs"><button class="${tab==='jornadas'?'active':''}" onclick="window.setTab('jornadas')">Jornada activa</button><button class="${tab==='clasificacion'?'active':''}" onclick="window.setTab('clasificacion')">General</button>${mine}<button class="${tab==='normas'?'active':''}" onclick="window.setTab('normas')">Normas</button><button class="${tab==='estadisticas'?'active':''}" onclick="window.setTab('estadisticas')">Estadisticas</button><button class="${tab==='historial'?'active':''}" onclick="window.setTab('historial')">Historial</button></div>`}
function showUserHistory(userId){selectedHistoryUserId=String(userId||'');render();setTimeout(()=>document.querySelector('#user-history-panel')?.scrollIntoView({behavior:'smooth',block:'start'}),50)}
function userHistoryPanelHtml(user){
  const rows=poolPredictions().filter(p=>String(p.user_id)===String(user.id)).map(p=>({p,m:matches.find(x=>String(x.id)===String(p.match_id))})).filter(x=>x.m&&pointsForPrediction(x.p,x.m)!==null).sort((a,b)=>resultSortTime(b.m)-resultSortTime(a.m)||new Date(b.m.match_date)-new Date(a.m.match_date));
  return `<div id="user-history-panel" class="history-panel"><div class="history-title"><h3>Historial de ${safe(user.nick)}</h3><button class="small red" onclick="window.showUserHistory('')">Cerrar</button></div>${rows.map(({p,m})=>{const pts=pointsForPrediction(p,m),extras=[];if(p.is_joker)extras.push('Joker');if(p.scorer_prediction)extras.push('G: '+safe(p.scorer_prediction));if(p.mvp_prediction)extras.push('MVP: '+safe(p.mvp_prediction));if(p.sent_off_prediction)extras.push('R: '+safe(p.sent_off_prediction));return `<div class="history-row"><div><b>${teamName(m.home_team)} ${m.real_home}-${m.real_away} ${teamName(m.away_team)}</b><span>${safe(m.group_name||'')} · ${new Date(m.match_date).toLocaleDateString('es-ES')}</span></div><div><b>${p.pred_home}-${p.pred_away}</b><span>${extras.join(' · ')||'Marcador'}</span></div><div><b>${pts}</b><span>pts</span></div></div>`}).join('')||'<p class="muted">Aun no tiene resultados puntuados en esta porra.</p>'}</div>`;
}
function rankingView(){if(!currentPool)return poolsView();const rows=poolUsers().filter(u=>u.role!=='admin').map(u=>({u,...userStats(u)})).sort((a,b)=>b.total-a.total||b.exact-a.exact);const selected=rows.find(r=>String(r.u.id)===String(selectedHistoryUserId));return `<div class="card classification-card"><h2>Clasificacion general</h2><p class="muted compact-hint">Pulsa un usuario para ver su historial.</p><div class="ranking-list">${rows.map((r,i)=>`<button type="button" class="ranking compact-ranking ranking-button ${selected&&String(selected.u.id)===String(r.u.id)?'selected':''}" onclick="window.showUserHistory('${safe(r.u.id)}')"><div class="rank-pos">${i+1}</div><div class="ranking-main">${avatar(r.u)}<b>${safe(r.u.nick)}</b>${rankingTags(r,i,rows.length)}<span class="ranking-meta">${r.played} jugados · ${r.exact} exactos · ${r.diff} dif · ${r.sg} signo${r.jokers?' · J '+r.jokers:''}${r.leagueTable?' · Liga '+r.leagueTable:''}${r.mix?' · MIX '+r.mix:''}</span></div><div class="ranking-points"><b>${r.total}</b><span>pts</span></div></button>`).join('')||'<p class="muted">Sin participantes.</p>'}</div>${selected?userHistoryPanelHtml(selected.u):''}${moneyLedgerHtml(moneyLedger())}${classificationTabs()}</div>`}
function myLeaguePredictionView(){if(!currentPool)return poolsView();if(!isLeagueCompetition())return rankingView();const pm=myMembership();const pick=parseLeaguePick(pm);return `<div class="card classification-card"><h2>Mi pronostico</h2>${leagueTableOpenForMember(pm)?'<div class="notice"><b>Todavia puedes modificarla en Pendientes.</b></div>':'<div class="notice"><b>Clasificacion guardada.</b> Ya no se puede modificar.</div>'}${pick.length?`<div class="league-pick-list">${pick.map((t,i)=>`<div><b>${i+1}</b><span>${safe(t)}</span></div>`).join('')}</div>`:'<p class="muted">Aun no has guardado tu clasificacion.</p>'}${classificationTabs()}</div>`}
function jornadasView(){if(!currentPool)return poolsView();const ledger=moneyLedger();const realRounds=competitionRounds();const allRounds=Array.from({length:isLeagueCompetition()?38:Math.max(realRounds.length,1)},(_,i)=>realRounds.find(r=>roundOrderKey(r.key)===i+1)||{key:'Jornada '+(i+1),matches:[],finished:false,firstDate:0,pending:true});const selected=selectedRoundKey?allRounds.find(r=>r.key===selectedRoundKey):activeRoundFrom(allRounds);if(selected)selectedRoundKey=selected.key;const ledgerRows=selected?ledger.byRound.get(selected.key):null;const rows=ledgerRows||roundRowsForDisplay(selected);const best=[...rows].sort((a,b)=>b.roundPts-a.roundPts||a.total-b.total)[0];const worst=[...rows].sort((a,b)=>a.roundPts-b.roundPts||b.total-a.total)[0];const selectedHasResult=roundHasResult(selected);return `<div class="card classification-card"><h2>Clasificacion de jornada</h2><p class="muted">Por defecto ves la jornada activa. La clasificacion completa esta en la pestana General.</p>${selected?`<div class="round-standing"><h3>${safe(selected.key)}${selected.pending?' · pendiente':''}</h3>${selected.matches?.length?`<div class="round-awards"><span>Mejor: <b>${safe(best?.u?.nick||'-')} +${best?.roundPts??0}</b></span><span>Peor: <b>${safe(worst?.u?.nick||'-')} +${worst?.roundPts??0}</b></span></div>${rows.map((r,i)=>`<div class="round-row"><div><b>${i+1}</b></div><div>${avatar(r.u)}<b>${safe(r.u.nick)}</b></div><div><span>+${r.roundPts}</span><small>jornada</small></div><div><span>${r.total}</span><small>general</small></div></div>`).join('')}${!selected.finished&&!selectedHasResult?'<p class="muted">Aun no hay resultados en esta jornada.</p>':''}`:'<p class="muted">Esta jornada aun no tiene partidos cargados.</p>'}</div>`:''}<div class="round-tab-grid">${allRounds.map(item=>`<button class="${item.key===selectedRoundKey?'active':''} ${item.matches?.length?'':'pending'}" onclick="window.setRoundTab('${safe(item.key)}')">${safe(roundShortLabel(item.key))}</button>`).join('')}</div>${roundMoneySettingsHtml()}${roundMoneyLedgerHtml(rows)}${classificationTabs()}</div>`}
...
function historyView(){return `<div class="card classification-card"><h2>Historial</h2><p class="muted">Resultados de mas nuevo a mas viejo.</p>${archivedMatches().map(m=>`<div class="match"><b>${teamName(m.home_team)} ${m.real_home} - ${m.real_away} ${teamName(m.away_team)}</b><div class="muted">${new Date(m.result_updated_at||m.match_date).toLocaleString('es-ES')}</div></div>`).join('')||'<p class="muted">Sin resultados recientes.</p>'}${currentUser?.role!=='admin'?classificationTabs():''}</div>`}
