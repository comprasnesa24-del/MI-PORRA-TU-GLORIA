...
function rankingView(){if(!currentPool)return poolsView();const rows=poolUsers().filter(u=>u.role!=='admin').map(u=>({u,...userStats(u)})).sort((a,b)=>b.total-a.total||b.exact-a.exact);return `<div class="card classification-card"><h2>Clasificacion</h2>${rows.map((r,i)=>`<div class="ranking"><div>${i+1}</div><div>${avatar(r.u)} ${safe(r.u.nick)}${rankingTags(r,i,rows.length)}<br><span class="muted">Exactos: ${r.exact} ? Dif: ${r.diff} ? Signo: ${r.sg} ? Joker: ${r.jokers} ? Goleadores: ${r.scorersOk} ? MVP: ${r.mvpsOk} ? Expulsion: ${r.sentOffsOk}</span></div><div>${r.total} pts</div></div>`).join('')||'<p class="muted">Sin participantes.</p>'}${classificationTabs()}</div>`}
