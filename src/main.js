...
// Bloquea SOLO cuando llegue la fecha real del partido
function isLocked(m) {
...
async function resetReal(mid) {
  if (!confirm('¿Seguro que quieres borrar el resultado real de este partido?')) return
  ...
  app.innerHTML = `
    <div class="app">
      ...
      <div class="card">
        ...
        <p>Usa el archivo <b>supabase.sql</b> para crear la base de datos.</p>
      </div>
    </div>
  `
}
      ...
      ${matches.map(m => {
        ...
        return `
          <div class="match">
            ...
            <div class="teams">
              ${safe(m.home_team)} vs ${safe(m.away_team)}
            </div>

            <div class="muted">
              ${new Date(m.match_date).toLocaleString('es-ES')}
            </div>

            <p>
              <span class="badge ${locked ? 'closed' : ''}">
                ${locked ? '🔒 Bloqueado' : 'Abierto'}
              </span>
              <span class="badge">
                ${pts === null ? 'Pendiente' : pts + ' pts'}
              </span>
            </p>

            <div class="score">
              <div>
                <label>${safe(m.home_team)}</label>
              ...
              <div>
                <label>${safe(m.away_team)}</label>
                <input ${locked && currentUser.role !== 'admin' ? 'disabled' : ''} type="number" min="0" id="pa_${m.id}" value="${p?.pred_away ?? ''}">
              </div>
            </div>

            <button class="small" onclick="window.savePrediction('${m.id}')">
              Guardar pronóstico
            </button>

            <span class="muted">
              Resultado real:
              <b>${m.real_home === null ? 'Pendiente' : m.real_home + ' - ' + m.real_away}</b>
            </span>
          </div>
        `
      }).join('')}
...
function rulesView() {
  return `
    <div class="card">
      ...
      <div class="rule">
        <h3>🎯 Resultado exacto: 5 puntos</h3>
        <p>Acertar los goles exactos de los dos equipos.</p>
      </div>

      <div class="rule">
        <h3>⚽ Diferencia correcta: 3 puntos</h3>
        <p>No es exacto, pero aciertas la diferencia de goles.</p>
        <p>Ejemplo: queda 1-0 y pusiste 2-1.</p>
      </div>

      <div class="rule">
        <h3>✅ Signo correcto 1 / X / 2: 2 puntos</h3>
        <p>Acertar ganador o empate, aunque no sea exacto ni diferencia.</p>
      </div>

      <div class="rule">
        <h3>❌ Fallo: 0 puntos</h3>
        <p>No aciertas quién gana ni el empate.</p>
      </div>

      <div class="rule">
        <h3>🔒 Bloqueo</h3>
        <p>Los pronósticos se bloquean automáticamente cuando llega la fecha y hora del partido.</p>
      </div>
    </div>
  `
}
...
function rankingView() {
  const rows = users.map(u => {
    ...
    return { nick: u.nick, total, exact, diff, sg }
  }).sort((a, b) =>
    b.total - a.total ||
    b.exact - a.exact ||
    b.diff - a.diff ||
      ...
      ${rows.map((r, i) => `
        <div class="ranking">
          ...
          <div>
            ${safe(r.nick)}<br>
            <span class="muted">
              Exactos: ${r.exact} · Dif: ${r.diff} · Signos: ${r.sg}
            </span>
          </div>
          <div>${r.total} pts</div>
        </div>
      ...
      ${matches.map(m => `
        <div class="adminrow">
          ...
          <div class="score">
            <div>
              <label>${safe(m.home_team)}</label>
              <input type="number" min="0" id="rh_${m.id}" value="${m.real_home ?? ''}">
            </div>
            ...
            <div>
              <label>${safe(m.away_team)}</label>
              <input type="number" min="0" id="ra_${m.id}" value="${m.real_away ?? ''}">
            </div>
          </div>

          <button class="small" onclick="window.saveReal('${m.id}')">
            Guardar resultado
          </button>

          <button class="small red" onclick="window.resetReal('${m.id}')">
            Reiniciar resultado
          </button>
        </div>
      `).join('')}
