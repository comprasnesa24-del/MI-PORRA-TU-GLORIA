...
function canAdminPool(){return currentUser?.role==='admin'||currentPool?.created_by===currentUser?.id||myMembership()?.role==='admin'}
...
function teamCodeForName(name){
  ...
  // 1) Primero: nombres EXACTOS de tus partidos en español.
  if(MATCH_TEAM_CODES[n])return MATCH_TEAM_CODES[n]

  // 2) Luego: alias español/inglés.
  if(TEAM_ALIASES[n])return TEAM_ALIASES[n]

  // 3) Luego: nombre guardado en Supabase team_players.
  const direct=teamPlayers.find(p=>normTeam(p.team_name)===n || normTeam(p.team_code)===n)
  if(direct?.team_code)return direct.team_code

  // 4) Último intento: coincidencia parcial segura.
...
function tabs(){const poolLabel=currentPool?`<div class="card"><b>Porra actual:</b> ${safe(currentPool.name)} · Código: <b>${safe(currentPool.code)}</b> · Goleador: <b>${currentPool.enable_scorer?'Sí':'No'}</b>${currentPool.prizes?`<br><br><b>🏆 Premios:</b><br>${safe(currentPool.prizes).replace(/\n/g,'<br>')}`:''}</div>`:'';const adminTabs=`<div class="tabs"><button onclick="window.setTab('porras')">Todas las porras</button><button class="yellow" onclick="window.setTab('resultados')">Resultados globales</button><button class="blue" onclick="window.setTab('archivo')">Archivo</button><button class="blue" onclick="window.setTab('historial')">Historial</button><button class="red" onclick="window.logout()">Salir</button></div>`;const userTabs=`<div class="tabs"><button onclick="window.setTab('porras')">Mis porras</button><button onclick="window.setTab('partidos')">Partidos</button><button class="blue" onclick="window.setTab('normas')">Normas</button><button class="blue" onclick="window.setTab('clasificacion')">Clasificación</button><button class="blue" onclick="window.setTab('estadisticas')">Estadísticas</button><button class="blue" onclick="window.setTab('chat')">Chat</button><button class="blue" onclick="window.setTab('historial')">Historial</button><button class="blue" onclick="window.setTab('archivo')">Archivo</button>${canAdminPool()?`<button class="yellow" onclick="window.setTab('admin')">Admin</button>`:''}<button class="red" onclick="window.logout()">Salir</button></div>`;return `${currentUser?.role==='admin'?adminTabs:userTabs}<div class="card"><h2>${avatar(currentUser)} Hola, ${safe(currentUser.nick)}</h2><p>${currentUser?.role==='admin'?'Modo administrador':'El Mundial pasa cada cuatro años; las bromas al último de la porra duran mucho más.'}</p><button class="small blue" onclick="window.saveProfile()">Editar avatar</button></div>${poolLabel}`}
function poolsView(){const myIds=currentUser?.role==='admin'?pools.map(p=>p.id):poolMembers.filter(pm=>pm.user_id===currentUser.id).map(pm=>pm.pool_id);const list=pools.filter(p=>myIds.includes(p.id));return `<div class="card"><h2>${currentUser?.role==='admin'?'Todas las porras privadas':'🏆 Mis porras privadas'}</h2>${currentUser?.role!=='admin'?`<p class="muted">Crea una porra o únete con código.</p><button onclick="window.createPool()">➕ Crear porra privada</button><button class="blue" onclick="window.joinPool()">🔑 Unirme con código</button>`:''}${list.map(p=>`<div class="match"><h3>${safe(p.name)}</h3><p>Código: <b>${safe(p.code)}</b> · Participantes: <b>${poolMembers.filter(pm=>pm.pool_id===p.id).length}</b> · Joker: <b>${p.enable_joker?'Sí':'No'}</b> · Goleador: <b>${p.enable_scorer?'Sí':'No'}</b></p>${p.prizes?`<p><b>🏆 Premios:</b><br>${safe(p.prizes).replace(/\n/g,'<br>')}</p>`:''}<button onclick="window.selectPool('${p.id}')">Entrar en esta porra</button>${currentUser?.role!=='admin'?`<button class="red" onclick="window.leavePool('${p.id}')">Salir de esta porra</button>`:''}</div>`).join('')||'<p class="muted">No hay porras.</p>'}</div>`}
...
function chatView(){if(!currentPool)return poolsView();const ms=messages.filter(m=>m.pool_id===currentPool.id);return `<div class="card"><h2>💬 Chat</h2><div class="match">${ms.map(m=>`<p><b>${safe(users.find(u=>u.id===m.user_id)?.nick||'Usuario')}:</b> ${safe(m.body)}</p>`).join('')||'<p class="muted">Sin mensajes.</p>'}</div><input id="chatmsg" placeholder="Mensaje"><button onclick="window.sendMessage()">Enviar</button></div>`}
async function sendMessage(){const body=document.querySelector('#chatmsg').value.trim();if(!body||!currentPool)return;await supabase.from('messages').insert({pool_id:currentPool.id,user_id:currentUser.id,body});await loadData()}
...
function archiveView(){const list=archivedMatches();return `<div class="card"><h2>📦 Archivo</h2>${list.map(m=>`<div class="match"><span class="group">${safe(m.group_name)}</span><div class="teams">${teamName(m.home_team)} vs ${teamName(m.away_team)}</div><p>Resultado: <b>${m.real_home===null?'Pendiente':m.real_home+' - '+m.real_away}</b></p>${currentPool?matchPoolPredictions(m.id):''}</div>`).join('')||'<p class="muted">Sin partidos archivados.</p>'}</div>`}
...
async function saveGlobalReal(mid){const rh=parseInt(document.querySelector('#grh_'+mid).value,10),ra=parseInt(document.querySelector('#gra_'+mid).value,10);if(isNaN(rh)||isNaN(ra)||rh<0||ra<0)return alert('Pon resultado válido');await supabase.from('matches').update({real_home:rh,real_away:ra,real_scorers:selectedRealScorers(mid)}).eq('id',mid);await loadData();alert('Resultado guardado')}
async function resetGlobalReal(mid){if(!confirm('¿Borrar resultado?'))return;await supabase.from('matches').update({real_home:null,real_away:null,real_scorers:''}).eq('id',mid);await loadData()}
function adminView(){if(!currentPool)return poolsView();return `<div class="card"><h2>Admin · ${safe(currentPool.name)}</h2><button class="yellow" onclick="window.updatePoolSettings()">⚙️ Joker / Goleador / Premios</button><h3>Participantes</h3>${poolUsers().map(u=>`<p>${avatar(u)} ${safe(u.nick)} ${u.role==='admin'?'(admin)':''}</p>`).join('')}</div>`}
async function updatePoolSettings(){if(!currentPool)return;const enable_joker=confirm('¿Activar Joker?');const enable_scorer=confirm('¿Activar goleador?');const prizes=prompt('Premios:',currentPool.prizes||'')||'';await supabase.from('pools').update({enable_joker,enable_scorer,prizes}).eq('id',currentPool.id);await loadData()}
...
Object.assign(window,{login,register,logout,setTab,selectPool,createPool,joinPool,leavePool,savePrediction,saveGlobalReal,resetGlobalReal,updatePoolSettings,saveProfile,sendMessage})
