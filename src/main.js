...
async function findLoginProfile(nick,email){const key=cleanLoginValue(nick),mail=cleanLoginValue(email);if(key){const byNick=await supabase.from('profiles').select('*').ilike('nick',key).limit(2);if(byNick.error)return{error:byNick.error};if((byNick.data||[]).length)return{data:byNick.data[0],many:byNick.data.length>1,mode:'nick'}}if(mail){const byEmail=await supabase.from('profiles').select('*').ilike('email',mail).limit(2);if(byEmail.error)return{error:byEmail.error};if((byEmail.data||[]).length)return{data:byEmail.data[0],many:byEmail.data.length>1,mode:'email'}}return{data:null}}
async function register(){const nick=cleanLoginValue(document.querySelector('#nick').value),email=cleanLoginValue(document.querySelector('#email').value),password=cleanLoginValue(document.querySelector('#pass').value);if(!nick||!password)return alert('Pon nick y contraseña');const exists=await supabase.from('profiles').select('id').ilike('nick',nick).limit(1).maybeSingle();if(exists.error)return alert(exists.error.message);if(exists.data)return alert('Ese nick ya existe, aunque cambies mayusculas o minusculas');let role='user';if(nick.toLowerCase()===ADMIN_NICK){if(password!==ADMIN_PASSWORD)return alert('Contraseña de admin incorrecta');role='admin'}const ins=await supabase.from('profiles').insert({nick,email,password,role,avatar:role==='admin'?'🛡️':'🙈'}).select().single();if(ins.error)return alert(ins.error.message);currentUser=ins.data;currentPool=null;tab='porras';saveSession();await loadData()}
