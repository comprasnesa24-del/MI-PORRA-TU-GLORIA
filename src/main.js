...
function userHistoryPanelHtml(user){
  const rows=poolPredictions().filter(p=>String(p.user_id)===String(user.id)).map(p=>({p,m:matches.find(x=>String(x.id)===String(p.match_id))})).filter(x=>x.m&&pointsForPrediction(x.p,x.m)!==null).sort((a,b)=>resultSortTime(b.m)-resultSortTime(a.m));
