...
function moneyLedgerHtml(ledger){if(!ledger.totals.length)return'';return `<div class="notice compact-card money-ledger"><h3>Cuenta acumulada</h3>${ledger.totals.map(r=>`<div><span>${avatar(r.u)}<b>${safe(r.u.nick)}</b></span><b class="${r.balance>=0?'money-good':'money-bad'}">${moneyFmt(r.balance)}</b></div>`).join('')}</div>`}
