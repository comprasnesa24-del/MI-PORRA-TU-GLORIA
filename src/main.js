...
function isClassificationTab(){return ['clasificacion','mi_pronostico','normas','estadisticas','historial'].includes(tab)}
function tabs(){
...
function classificationTabs(){const mine=currentCompetition==='liga'&&leagueStarted()?`<button class="${tab==='mi_pronostico'?'active':''}" onclick="window.setTab('mi_pronostico')">Mi pronostico</button>`:'';return `<div class="section-tabs"><button class="${tab==='clasificacion'?'active':''}" onclick="window.setTab('clasificacion')">Clasificacion</button>${mine}<button class="${tab==='normas'?'active':''}" onclick="window.setTab('normas')">Normas</button><button class="${tab==='estadisticas'?'active':''}" onclick="window.setTab('estadisticas')">Estadisticas</button><button class="${tab==='historial'?'active':''}" onclick="window.setTab('historial')">Historial</button></div>`}
...
function render(){document.title='MI PORRA';if(!currentUser){app.innerHTML=loginView();return}let content='';try{content=tab==='porras'?poolsView():tab==='partidos'?matchesView():tab==='normas'?rulesView():tab==='clasificacion'?rankingView():tab==='mi_pronostico'?myLeaguePredictionView():tab==='estadisticas'?statsView():tab==='chat'?chatView():tab==='historial'?historyView():tab==='archivo'?archiveView():tab==='resultados'?globalResultsView():tab==='admin'?adminView():poolsView()}catch(e){console.error(e);content=`<div class="card"><h2>Error</h2><p>${safe(e.message)}</p><button onclick="window.setTab('porras')">Volver</button></div>`}app.innerHTML=`<div class="app has-bottom-nav">${tabs()}<main class="screen">${content}</main>${podiumReminderHtml()}</div>`}
