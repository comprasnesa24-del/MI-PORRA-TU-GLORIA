...
function teamPickSelectHtml(id,value){return `<select id="${id}" class="podium-select"><option value="">Sin elegir</option>${tournamentTeams().map(t=>`<option value="${safe(t.name)}" ${normTeam(value)===normTeam(t.name)?'selected':''}>${safe(t.name)}</option>`).join('')}</select>`}
...
function podiumPredictionHtml(){const pm=myMembership();if(!currentPool||!pm||currentUser?.role==='admin')return'';return `<div class="card podium-card"><h2>Podio del Mundial</h2><p class="muted">Campeon +20 ? Segundo +15 ? Tercero +10</p><div class="podium-grid"><label>Campeon${teamPickSelectHtml('pick_champion',pm.champion_pick)}</label><label>Segundo${teamPickSelectHtml('pick_second',pm.second_pick)}</label><label>Tercero${teamPickSelectHtml('pick_third',pm.third_pick)}</label></div><button class="yellow" onclick="window.savePodiumPrediction()">Guardar podio</button></div>`}
async function savePodiumPrediction(){if(!currentPool||!currentUser)return;const champion=document.querySelector('#pick_champion')?.value||null,second=document.querySelector('#pick_second')?.value||null,third=document.querySelector('#pick_third')?.value||null;const picks=[champion,second,third].filter(Boolean).map(normTeam);if(new Set(picks).size!==picks.length)return alert('No repitas equipo en el podio');const res=await supabase.from('pool_members').update({champion_pick:champion,second_pick:second,third_pick:third}).eq('pool_id',currentPool.id).eq('user_id',currentUser.id);if(res.error)return alert(res.error.message);await loadData();alert('Podio guardado')}
function adminPodiumHtml(){if(!currentPool)return'';return `<div class="card podium-card"><h2>Podio real del Mundial</h2><p class="muted">Estos valores suman puntos en esta porra.</p><div class="podium-grid"><label>Campeon${teamPickSelectHtml('real_champion',currentPool.real_champion)}</label><label>Segundo${teamPickSelectHtml('real_second',currentPool.real_second)}</label><label>Tercero${teamPickSelectHtml('real_third',currentPool.real_third)}</label></div><button class="yellow" onclick="window.saveRealPodium()">Guardar podio real</button></div>`}
...
Object.assign(window,{login,register,logout,setTab,changePool,selectPool,createPool,joinPool,leavePool,deletePool,savePrediction,saveGlobalReal,resetGlobalReal,updatePoolSettings,saveProfile,sendMessage,refreshChat,savePodiumPrediction,saveRealPodium})
